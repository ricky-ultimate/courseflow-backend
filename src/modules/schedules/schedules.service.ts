...
> {
  ...
  protected async beforeCreate(
    ...
    const sessionType = dto.sessionType ?? SessionType.THEORY;

    if (
      this.schedulerEngine.hasConflictWithFixedFridaySlot(
        dto.courseCode,
        course.semester,
        dto.dayOfWeek,
        dto.startTime,
      ...
      throw new ConflictException(
        `Time slot ${dto.startTime}-${dto.endTime} on ${dto.dayOfWeek} conflicts with the fixed Friday slot for course '${dto.courseCode}'.`,
      );
    }

    await this.assertDistinctSessionTypeDay(
      dto.courseCode,
      activeSession.id,
      sessionType,
      dto.dayOfWeek,
    ...
    if (existing) {
      throw new ConflictException(
        `Course '${dto.courseCode}' already has a ${sessionType} schedule this session. Update the existing one.`,
      );
    }
  ...
  protected async beforeUpdate(
    ...
    const current = await this.prisma.schedule.findUnique({
      where: { id: identifier },
      include: {
        course: { select: { isGeneral: true, level: true, semester: true } },
      },
    });
    ...
    const resolvedSessionType = dto.sessionType ?? current.sessionType;
    ...
    if (
      ...
      throw new ConflictException(
        `Time slot ${resolvedStart}-${resolvedEnd} on ${resolvedDay} conflicts with the fixed Friday slot for course '${current.courseCode}'.`,
      );
    }
    ...
    if (dto.sessionType && dto.sessionType !== current.sessionType) {
      ...
      if (conflict) {
        throw new ConflictException(
          `Course '${current.courseCode}' already has a ${dto.sessionType} schedule this session.`,
        );
      }
    }
      ...
      if (assignments.length > 0) {
        await tx.schedule.createMany({
          data: assignments.map((a) => ({
            ...
            semester: a.semester,
            sessionId: session.id,
            sessionType: a.sessionType ?? SessionType.THEORY,
            isAutoGenerated: true,
            isManualOverride: false,
            isFixed: false,
          })),
  ...
  private async assertDistinctSessionTypeDay(
    courseCode: string,
    sessionId: string,
    ...
    const counterpart = await this.prisma.schedule.findFirst({
      where: {
        courseCode,
        sessionId,
        sessionType: { not: sessionType },
        ...(excludeScheduleId ? { id: { not: excludeScheduleId } } : {}),
      },
    });

    if (counterpart && counterpart.dayOfWeek === dayOfWeek) {
      throw new ConflictException(
        `Course '${courseCode}' already has a ${counterpart.sessionType} session on ${dayOfWeek}. Theory and practical sessions must be scheduled on different days.`,
      );
    }
  }
  ...
  private async checkUniversityCourseClash(
    ...
    const generalSchedules = await this.prisma.schedule.findMany({
      where: {
        sessionId,
        dayOfWeek,
        ...(excludeScheduleId && { id: { not: excludeScheduleId } }),
    ...
    if (generalSchedules.length === 0) return;

    const candidateStart = this.timeToMinutes(startTime);
    const candidateEnd = this.timeToMinutes(endTime);

    for (const general of generalSchedules) {
      const generalStart = this.timeToMinutes(general.startTime);
      const generalEnd = this.timeToMinutes(general.endTime);

      const overlaps =
        candidateStart < generalEnd && generalStart < candidateEnd;

      if (overlaps) {
        throw new ConflictException(
          `Time slot ${startTime}-${endTime} on ${dayOfWeek} conflicts with university course ` +
            `${general.course.code} (${general.course.name}) scheduled at ${general.startTime}-${general.endTime}. ` +
